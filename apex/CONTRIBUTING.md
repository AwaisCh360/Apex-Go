...
Thank you for your interest in contributing to Apex! This guide will help you get started with development and contributions.
...
1. **Clone the repository**
   ```bash
   git clone https://github.com/useapex/apex.git
   cd apex
   ```
...
4. **Run Apex in development mode**
   ```bash
   uv run apex --target https://example.com
   ```

## 📚 Contributing Skills

Skills are specialized knowledge packages that enhance agent capabilities. See [apex/skills/README.md](apex/skills/README.md) for detailed guidelines.
...
`apex view` serves a prebuilt web UI whose source lives in
`apex/interface/viewer/frontend/` (a Vite + React project) and whose built output is
committed to `apex/interface/viewer/static/` and shipped in the package. End users never
run a JS build. If you change anything under `apex/interface/viewer/frontend/`, rebuild
and commit the output:

```bash
make viewer   # or: cd apex/interface/viewer/frontend && npm ci && npm run build
```

Commit both the source change and the regenerated `apex/interface/viewer/static/`.
...
`apex/bin/apex-tui`, and assigns the current platform tag. It requires Go
1.24.x or newer and fails rather than producing a wheel without the sidecar.
`scripts/build.sh` and `apex.spec` are likewise strict for frozen PyInstaller
releases.

## 🤝 Community

- **Discord**: [Join our community](https://discord.gg/apex-ai)
- **Issues**: [GitHub Issues](https://github.com/useapex/apex/issues)
...
**Questions?** Reach out on [Discord](https://discord.gg/apex-ai) or create an issue. We're here to help!
