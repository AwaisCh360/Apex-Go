...
func TestSetupStartScreenFitsNarrowTerminal(t *testing.T) {
	...
	if !strings.Contains(view, topRow) && !strings.Contains(view, "APEX") {
...
func TestModalKeepsBackgroundVisible(t *testing.T) {
	...
	if !strings.Contains(view, "Apex Help") {
...
func TestBudgetPauseShowsOneWarningToastUntilResumed(t *testing.T) {
	model := New(nil)
	model.snapshot.Agents = []protocol.Agent{{ID: "root", Name: "Apex", Status: "budget_paused"}}
