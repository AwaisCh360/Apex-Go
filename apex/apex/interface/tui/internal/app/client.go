...
func ConnectInherited(fdValue string) (*Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("invalid APEX_TUI_FD: %w", err)
	}
	file := os.NewFile(uintptr(fd), "apex-tui-ipc")
	if file == nil {
		return nil, fmt.Errorf("invalid APEX_TUI_FD %d", fd)
	}
...
func ConnectFromEnvironment() (*Client, error) {
	if fd := os.Getenv("APEX_TUI_FD"); fd != "" {
		_ = os.Unsetenv("APEX_TUI_FD")
		return ConnectInherited(fd)
	}

	address := os.Getenv("APEX_TUI_ADDR")
	token := os.Getenv("APEX_TUI_TOKEN")
	_ = os.Unsetenv("APEX_TUI_ADDR")
	_ = os.Unsetenv("APEX_TUI_TOKEN")
	if address == "" || token == "" {
		return nil, fmt.Errorf("APEX_TUI_FD or APEX_TUI_ADDR and APEX_TUI_TOKEN are required")
	}
