...
import (
	...
	"github.com/useapex/apex/tui/internal/protocol"
)
...
func TestConnectFromEnvironmentAuthenticatesTCPTransport(t *testing.T) {
	...
	t.Setenv("APEX_TUI_ADDR", listener.Addr().String())
	t.Setenv("APEX_TUI_TOKEN", "one-use-token")
	t.Setenv("APEX_TUI_FD", "")
	...
	if os.Getenv("APEX_TUI_ADDR") != "" || os.Getenv("APEX_TUI_TOKEN") != "" {
...
func TestConnectFromEnvironmentRequiresCompleteTransport(t *testing.T) {
	t.Setenv("APEX_TUI_FD", "")
	t.Setenv("APEX_TUI_ADDR", "127.0.0.1:1")
	t.Setenv("APEX_TUI_TOKEN", "")

	_, err := ConnectFromEnvironment()
	if err == nil || !strings.Contains(err.Error(), "APEX_TUI_ADDR and APEX_TUI_TOKEN") {
		t.Fatalf("error = %v, want missing transport error", err)
	}
}

func TestConnectFromEnvironmentPrefersInheritedDescriptor(t *testing.T) {
	t.Setenv("APEX_TUI_FD", "not-a-number")
	t.Setenv("APEX_TUI_ADDR", "127.0.0.1:1")
	t.Setenv("APEX_TUI_TOKEN", "token")

	_, err := ConnectFromEnvironment()
	if err == nil || !strings.Contains(err.Error(), "invalid APEX_TUI_FD") {
